...
async function buttonClick() {
  ...
  await axios.put(`/api/todoitem/${router.currentRoute.value.params.id}`, dataToServer)
  router.replace('/todoItem')
}

async function init() {
  const res = await axios.get<TodoItem>(`/api/todoitem/${router.currentRoute.value.params.id}`)
